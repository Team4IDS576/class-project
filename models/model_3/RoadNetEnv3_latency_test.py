...
class raw_env(AECEnv):
    ...
    def state(self) -> np.ndarray:
        ...
        # latency check - latencies do not update and reset doesnt work
        data = tuple(self.road_network.edges(data=True))
        latencies = []
        # print(data[0][:2]) # network doesn't update
        
        for link in data:
            latencies.append(link[-1]["latency"])
        
        # return list(self.agent_travel_time.values())
        # return list(self.agent_wait_time.values())
        return latencies
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            if (
                self.agent_locations[agent_idx] != self.agent_destinations[agent_idx] and
