...
def nguyenNetwork(high=True):
    ...
    # for mac OS
    if high == True:
        links = pd.read_csv("./network/NguyenLinksHighDemand.csv", dtype=data_types)
    else:
        links = pd.read_csv("./network/NguyenLinksLowDemand.csv", dtype=data_types)
...
def traffic(high=True):
    if high == True:
        demand = pd.read_csv("./network/NguyenDemandHighDemand.csv")
    else:
        demand = pd.read_csv("./network/NguyenDemandLowDemand.csv")
    agents = []
    origins = []
    destinations = []
    agent_no = 0
